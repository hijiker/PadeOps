    ...
    subroutine RegularizeFneq_BC(this)
        ...
        this%PiBC = zero
        do vid = 1,nvels
            this%PiBC(:,:,1) = this%PiBC(:,:,1) + this%Qtensor(1,1,vid)*this%fneqBC(:,:,vid) 
            this%PiBC(:,:,2) = this%PiBC(:,:,2) + this%Qtensor(1,2,vid)*this%fneqBC(:,:,vid) 
            this%PiBC(:,:,3) = this%PiBC(:,:,3) + this%Qtensor(1,3,vid)*this%fneqBC(:,:,vid) 
            this%PiBC(:,:,4) = this%PiBC(:,:,4) + this%Qtensor(2,2,vid)*this%fneqBC(:,:,vid) 
            this%PiBC(:,:,5) = this%PiBC(:,:,5) + this%Qtensor(2,3,vid)*this%fneqBC(:,:,vid) 
            this%PiBC(:,:,6) = this%PiBC(:,:,6) + this%Qtensor(3,3,vid)*this%fneqBC(:,:,vid) 
        end do 
   
        do vid = 1,nvels
            this%fneqBC(:,:,vid) = (w(vid)*oneby2c4)*(this%PiBC(:,:,1)*this%Qtensor(1,1,vid) + two*this%PiBC(:,:,2)*this%Qtensor(1,2,vid) &
                                 &  + two*this%PiBC(:,:,3)*this%Qtensor(1,3,vid) + this%PiBC(:,:,4)*this%Qtensor(2,2,vid)   &
                                 &  + two*this%PiBC(:,:,5)*this%Qtensor(2,3,vid) + this%PiBC(:,:,6)*this%Qtensor(3,3,vid) ) & 
