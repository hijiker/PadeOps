...
subroutine doTemporalStuff(bgp)
    ...
    usum = p_sum(sum(bgp%ux))/real(bgp%nx*bgp%ny,rkind) 
    vsum = p_sum(sum(bgp%uy))/real(bgp%nx*bgp%ny,rkind)
    wsum = p_sum(sum(bgp%uz))/real(bgp%nx*bgp%ny,rkind)

    call message(0,"Time:", bgp%getPhysTime())
    call message(0,"TIDX:", bgp%step)
    call message(1,"u_tau:", utau)
    call message(1,"u_match:", umatch)
    call message(1,"u_sum:", usum)
    call message(1,"v_sum:", vsum)
    call message(1,"w_sum:", wsum)
